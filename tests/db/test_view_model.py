...
from mitzasql.db.model import ViewModel
from .connection_fixture import sakila_connection

def test_model_fetches_data(sakila_connection):
    model = ViewModel(sakila_connection, 'sakila', 'customer_list')
...
def test_model_is_empty(sakila_connection):
    model = ViewModel(sakila_connection, 'sakila', 'unknown view')

    assert model.last_error is None
    assert len(model) == 0

def test_model_does_not_raise_exception_on_error(sakila_connection):
    model = ViewModel(sakila_connection, 'sakila\' FROM x', 'customer_create_date')
